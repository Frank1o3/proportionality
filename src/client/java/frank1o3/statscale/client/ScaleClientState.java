...
/**
 * Client-side singleton that caches the player's current scale and the server-advertised
 * maximum scale received via {@link ScaleSyncPayload}.
 *
 * <p>This class is intentionally simple: it is only written on the client thread
 * (Fabric's networking callbacks marshal onto the render thread for client receivers)
 * and only read by GUI code that also runs on the render thread, so no synchronisation
 * is needed.
 *
 * <h2>Lifecycle</h2>
 * <ul>
 *   <li>Populated when the server sends a {@link ScaleSyncPayload} (on login and after
...
public final class ScaleClientState {
    ...
    /** Default server max used before the server has sent its first sync (singleplayer / offline). */
    public static final float DEFAULT_MAX_SCALE = 16.0f;

    private static float currentScale    = DEFAULT_SCALE;
    private static float serverMaxScale  = DEFAULT_MAX_SCALE;
     ...
     * Used to cap the slider range in {@link frank1o3.statscale.client.gui.ScaleScreen}.
     * Defaults to {@link #DEFAULT_MAX_SCALE} until the server sends a sync packet.
     */
    public static float getServerMaxScale() {
     ...
     * Called on the client thread by {@link frank1o3.statscale.network.ClientScaleNetwork}.
     *
     * @param scale    The player's active (persisted) scale value.
     * @param maxScale The server's configured maximum scale.
     */
    public static void applySync(float scale, float maxScale) {
        currentScale   = scale;
